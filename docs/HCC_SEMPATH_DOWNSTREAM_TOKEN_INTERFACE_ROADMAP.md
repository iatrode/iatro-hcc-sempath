...
input_validity       [B]   # INTERNAL diagnostic only — NOT exported to the token record
```

This package is the **internal** frozen feature package consumed by the downstream adapter; it is
not the downstream token contract. In particular `input_validity` stays internal: validity is not
an exported representation field. Structurally empty inputs should be filtered before inference,
and relevance or query-conditioned quality should be handled by the downstream consumer.
...
learned pathology queries
    ...
    -> projection to a shared downstream token plane (D = 1024)
    -> pathology_tokens [B, K_token, 1024]
    -> pathology_mask   [B, K_token]
```

Downstream token contract:

- `D = 1024` is the proposed public token dimension for downstream integration. The adapter should
  target this dimension directly rather than adding an unsupervised post-hoc dimension patch.
- No single-vector summary field is required in the exported contract. If a downstream system needs
  a routing or fusion readout, that readout should be derived from the delivered tokens and mask in
  that system's own connector.
- `K_token` is HCC-SemPath's own token count. Downstream consumers should treat token count as a
  per-source interface parameter rather than a global constant.
- The exposed expert record is `{expert_name, tokens [K_token,1024], mask [K_token], provenance}`
  and carries no `input_validity`.
...
- select `K_token` before downstream evaluation; target dimension is fixed at `D = 1024` by the
  exported token contract, not a free choice;
- a passing interface check at this stage is a **tensor interface smoke** only (shapes
  `[B,K_token,1024]` / `[B,K_token]`); the full expert-record contract smoke (incl. provenance fields)
