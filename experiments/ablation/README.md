...
A1-A11 reuse its exact 10% population subset, fixed classification/spatial
...
A11 retains adjudication but removes only student-response matching. Together,
A0, A2, and A11 isolate the two PAMT-D mechanisms. A5 and A6
retain identical module topology and differ only in whether the corresponding
complete-bank student centroids refresh. A8-A10 also retain identical
parameters and output geometry; their named computation path is bypassed.

The mechanism matrix has three layers. A1--A4 and A11 test the central
multi-teacher/global-intervention/PAMT-D hypothesis. A5 and A6 test dynamic
prototype refresh, while A8--A10 test the spatial
...
denominators make it invalid as a cross-condition efficacy endpoint. The
reported mechanism comparison uses the teacher-retention diagnostic and the
...
evaluation and are not rerun separately for every ablation.
...
```bash
HCC_SEMPATH_ABLATION_BASE_CONFIG=/path/to/best_trial/config.yaml \
  bash experiments/ablation/scripts/run_ablations.sh a2 a8 a9 a10 a11
```
...
bash scripts/run_a11_nvme.sh
# After A11 has completed and the full run is approved:
