...
function prototypeButton(level,id,selected,count,maxCount,onClick,shortcut=''){const b=document.createElement('button'); const pct=maxCount?Math.round(count/maxCount*100):0; b.className='chip'+(selected?' selected':''); b.style.setProperty('--pct',pct+'%');const key=shortcut?`<kbd class=chipShortcut>${shortcut}</kbd>`:'';b.innerHTML=`<span class=chipRow><span>${key}${labelName(level,id)}</span><span class=chipCount>${count}</span></span>`; b.onclick=onClick; return b;}
...
document.addEventListener('keydown',ev=>{const mod=ev.metaKey||ev.ctrlKey,target=ev.target,editing=target&&(target.matches?.('input,textarea,select')||target.isContentEditable);if(MODE==='l1'&&!mod&&!editing&&!document.querySelector('dialog[open]')&&/^[1-9]$/.test(ev.key)){const index=Number(ev.key)-1;if(index<L1.length){ev.preventDefault();l1=L1[index];renderLabels()}return}if(!mod)return;if(ev.key.toLowerCase()==='z'){ev.preventDefault();if(ev.shiftKey)redoRoi();else undoRoi()}else if(ev.key.toLowerCase()==='y'){ev.preventDefault();redoRoi()}});
