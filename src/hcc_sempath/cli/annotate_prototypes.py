...
function applyDefaultTileZoom(){if(tileZoomUserSet)return;if(!isMobile()){setTileZoom(2);return}const viewport=document.getElementById('tileViewport');if(!viewport)return;const available=Math.min(viewport.clientWidth,viewport.clientHeight);if(available>0)setTileZoom(available/224)}
