...
function toggleRoiClass(name){pushUndo();roiPlan=null;if(roiAttribute===name){roiClassState[name]=roiClassState[name]==='negative'?'open':'negative'}else{roiAttribute=name;if(!roiClassState[name])roiClassState[name]='open'}renderLabels();renderRoi()}
...
function similarityThreshold(){return parseFloat(document.getElementById('roiSimilarity')?.value||'0.85')}
function visiblePlannedSuggestions(){return(roiPlan?.suggestions||[]).filter(item=>(item.confidence??0)>=similarityThreshold())}
function renderRoiPlan(){const status=document.getElementById('roiPlanStatus'),decision=document.getElementById('roiPlanDecision'),button=document.getElementById('roiPlanGenerate'),control=document.getElementById('roiSimilarityControl');if(!status)return;button.disabled=roiPlanLoading||!current;if(roiPlanLoading){button.textContent='Finding matches…';status.textContent='Comparing local image patches with the selected seed marks.';decision.classList.add('hidden');control.classList.add('hidden');return}button.textContent=roiPlan?'Rebuild similar marks':'Find similar marks';if(!roiPlan){const seeds=seedCentersForSelectedClass();status.textContent=roiAttribute===ROI_ALL?'Select one ROI class, then add 1–3 reliable point or circle seeds.':`${labelName('l2',roiAttribute)} · ${seeds.length} seed${seeds.length===1?'':'s'}. Add a reliable mark, then find similar centers.`;decision.classList.add('hidden');control.classList.add('hidden');return}const visible=visiblePlannedSuggestions();document.getElementById('roiSimilarityValue').textContent=`${Math.round(similarityThreshold()*100)}%`;status.textContent=`Similarity preview · ${roiPlan.summary?.seed_count||0} seeds · showing ${visible.length}/${roiPlan.suggestions.length} relative matches. Outlined marks are not saved yet.`;decision.classList.remove('hidden');control.classList.remove('hidden')}
function drawPlannedPoint(x,g,color){const px=g.point[0]*224,py=g.point[1]*224;x.save();x.globalAlpha=1;x.setLineDash([]);for(const [radius,fill] of [[7,'rgba(15,23,42,.95)'],[5.5,'#fff'],[4,color]]){x.fillStyle=fill;x.beginPath();x.arc(px,py,radius,0,Math.PI*2);x.fill()}x.restore()}
...
async function generateRoiPlan(){if(!ROI_MODE||!current||roiPlanLoading)return;if(roiAttribute===ROI_ALL){document.getElementById('status').textContent='Select one ROI class before finding similar marks.';return}const seeds=seedCentersForSelectedClass();if(!seeds.length){document.getElementById('status').textContent='Add at least one reliable point or circle seed for the selected class first.';return}const tileKey=`${pkg}:${current.row}:${current.tile_id}`;roiPlan=null;roiPlanLoading=true;renderRoi();try{const result=await api('/api/roi-similar',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({package:pkg,row:current.row,attribute:roiAttribute,seeds})});if(!current||tileKey!==`${pkg}:${current.row}:${current.tile_id}`)return;roiPlan=result;document.getElementById('status').textContent='Similar local image centers are ready. Adjust the threshold, then accept visible matches or start from scratch.';}catch(e){document.getElementById('status').textContent='Similarity search failed: '+(e.message||String(e))}finally{roiPlanLoading=false;renderRoi()}}
...
function clearSelectedRoiClass(){if(roiAttribute===ROI_ALL){document.getElementById('status').textContent='Select one L2 ROI class before clearing.';return}const cleared=roiAttribute;pushUndo();roi=roi.filter(x=>x.attribute!==cleared);delete roiClassState[cleared];roiDrawing=null;roiPreview=null;roiCursor=null;roiPlan=null;syncPositiveLabels();renderRoi();document.getElementById('status').textContent=`Cleared ROI state for: ${labelName('l2',cleared)}.`}
        ...
        def do_POST(self):  # noqa: N802
            ...
            if parsed.path == "/api/roi-similar":
                try:
                    ...
                    _json_response(
                        self,
                        roi_plan_generator.generate_similar(
                            tile_png,
                            attribute=attribute,
