...
def _discover_teacher_paths(root: Path | None, teachers: list[str]) -> dict[str, list[Path]]:
    ...
    for teacher in teachers:
        ...
        if not teacher_matches:
            raise FileNotFoundError(f"no feature packages found for teacher={teacher} under {root}")
...
def _resolve_teacher_paths(args: argparse.Namespace) -> dict[str, list[Path]]:
    ...
    paths = _discover_teacher_paths(Path(args.teacher_feature_root) if args.teacher_feature_root else None, teachers)
...
def _plot_curves(
    *,
    ...
    panels = [
        ("delta_coverage_mean", "Delta coverage", axes[0, 0]),
        ("val_agreement_mean", "Validation agreement", axes[0, 1]),
    ...
    for fmt in formats:
        path = figure_dir / f"anchor_information_summary.{fmt}"
    ...
    for level, suffix in [(1, "level1"), (2, "level2")]:
        ...
        for fmt in formats:
            path = figure_dir / f"anchor_information_{suffix}_prototypes.{fmt}"
            fig.savefig(path, dpi=180)
            written.append(str(path))
        plt.close(fig)
    ...
    if len(teacher_names) > 1:
        ...
        for fmt in formats:
            path = figure_dir / f"anchor_information_teacher_coverage.{fmt}"
            fig.savefig(path, dpi=180)
            written.append(str(path))
        plt.close(fig)
    return written
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.annotation_json:
        ...
        anchors, locked_val = _split_anchors(
            all_anchors,
            locked_val_fraction=float(args.locked_val_fraction),
            locked_val_count=int(args.locked_val_count),
        ...
        generated_manifest_info = {
            ...
            "locked_val_fraction": float(args.locked_val_fraction),
            "locked_val_count_requested": int(args.locked_val_count),
    ...
    subsets = _nested_subsets(anchors, anchor_counts, int(args.seed), str(args.anchor_group_key))
    ...
    _write_json(
        output_root / "anchor_information_plan.json",
        {
            ...
            "seed": int(args.seed),
            "split_seed": int(args.split_seed),
            "anchor_group_key": str(args.anchor_group_key),
    ...
    aggregate_rows = _aggregate_rows(
        teacher_rows,
        plateau_delta_epsilon=float(args.plateau_delta_epsilon),
    ...
    figure_paths = [] if args.no_plots else _plot_curves(
        output_root=output_root,
        teacher_rows=teacher_rows,
        aggregate_rows=aggregate_rows,
        prototype_rows=prototype_rows,
        formats=_parse_str_list(args.plot_formats),
    )
    _write_json(output_root / "anchor_information_recommendation.json", {**recommendation, "figures": figure_paths})
    print(
        "anchor_information_curve_ok "
        f"output_root={output_root} teachers={len(teacher_paths)} counts={len(subsets)} "
        f"recommended_anchor_count={recommendation['recommended_anchor_count']}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--anchor-counts", default="100,200,400,800,1200,1600,2000,3000")
    ...
    parser.add_argument("--locked-val-fraction", type=float, default=0.2)
    parser.add_argument("--locked-val-count", type=int, default=0)
    parser.add_argument("--anchor-group-key", choices=["tile_id", "slide_id", "patient_id", "center"], default="tile_id")
    parser.add_argument("--bootstrap-iterations", type=int, default=500)
    parser.add_argument("--plateau-delta-epsilon", type=float, default=0.005)
    parser.add_argument("--plateau-drift-threshold", type=float, default=0.01)
    parser.add_argument("--plateau-redundancy-threshold", type=float, default=0.95)
